...
{
	{
		...
		auto latte = make_unique<CLatte>(2);
		// добавляем корицы
	...
	//Только латте
	{
		auto beverage = make_unique<CLatte>(1);
	...
	//Двоеное латте
	{
		auto beverage = make_unique<CLatte>(2);
	...
	//Только капучино
	{
		auto beverage = make_unique<CCapuccino>(1);
	...
	//Двойное капучино
	{
		auto beverage = make_unique<CCapuccino>(2);
	...
	//Иван чай
	{
		auto beverage = make_unique<СFireweed>();
	...
	//Пу-эр
	{
		auto beverage = make_unique<СPuerh>();
	...
	//Пу-эр с лимоном
	{
		auto beverage = make_unique<CLemon>(make_unique<СPuerh>());

		cout << beverage->GetDescription() << " costs " << beverage->GetCost() << endl;
	}
	//Молочный коктель маленький
	{
		auto beverage = make_unique<CMilkshake>(SizeOfCoctale::small);

		cout << beverage->GetDescription() << " costs " << beverage->GetCost() << endl;
	}

	//Молочный коктель средний
	{
		auto beverage = make_unique<CMilkshake>(SizeOfCoctale::normal);

		cout << beverage->GetDescription() << " costs " << beverage->GetCost() << endl;
	}
	//Молочный коктель большой с ликером
	{
		auto beverage = make_unique<CHazelnutLiquor>(make_unique<CMilkshake>(SizeOfCoctale::big));
